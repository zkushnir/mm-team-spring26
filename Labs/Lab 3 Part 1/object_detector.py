...
class YOLOEObjectDetector(Node):
    ...
    def get_goal_pose(self, detections, target_idx=0):
        ...
        # --- Part 1: depth at the 2D centroid ---

        # Sample the depth image at the centroid
        cx, cy = centroid
        z_depth = float(self.latest_depth[cy, cx])

        # Reject invalid depth readings
        if z_depth == 0.0:
            print(f"Invalid depth at centroid ({cx}, {cy}), skipping goal pose update")
            self.goal_pose_msg = None
            return None

        # Project the 2D centroid & depth to a 3D point
        xyz_camera = detection_utils.pixel_to_3d(
            xy_pix=centroid,
            z_depth=z_depth,
