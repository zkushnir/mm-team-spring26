...
class IKTargetFollowing(HelloNode):
    ...
    def goal_callback(self, goal_msg):
        ...
        # set up a state machine so that if the robot is too far it moves to a waypoint closer to the goal,
        # and if its close enough it goes directly to the goal and executes sequence to grasp the object
        if self._grasp_complete:
            return
        ...
        if distance_to_goal > GRASP_THRESHOLD:
            ...
            if not np.isclose(err, 0.0, atol=1e-2):
                print(f"IK position error is too large, skipping")
                q_soln = None
            ik.print_q(q_soln)
        ...
        # case 2: close enough -> do one final committed approach to center grasp before closing
        direction = goal_pos - gripper_pos
        norm = np.linalg.norm(direction)
        if norm > 1e-6:
            direction = direction / norm
        final_goal = goal_pos + 0.01 * direction

        q_soln = ik.chain.inverse_kinematics(
            final_goal,
            initial_position=q_init
        )
        err = np.linalg.norm(ik.chain.forward_kinematics(q_soln)[:3, 3] - final_goal)
        if np.isclose(err, 0.0, atol=2e-2):
            ik.print_q(q_soln)
            ik.move_to_configuration(self, q_soln)
        ...
        # lift object
        current_lift = self.joint_state.get('joint_lift', 0.8)
        self.move_to_pose({'joint_lift': min(current_lift + 0.15, 1.1)}, blocking=True)
        
        # move back with object
    ...
    def compute_waypoint_to_goal(self, goal_pos, gripper_pos):
        ...
        step = max(self.delta, 0.05)
        if distance > step:
            # waypoint to goal position 
            direction    = difference / distance
            waypoint_pos = gripper_pos + step * direction
        else:
